...
\lstinputlisting[language=Python, style=python]{source.py}
...
\begin{algorithm}
	...
	\State{{\textbf{Input}} array $A$, positive integers $i$, $j$ between $0$ and $n-1$}
	\If{$j - i \leq 1$} \Comment{length $0$ or $1$, so already sorted}
	  \State{}%
	  \Return{}
	\EndIf
	\State{$mid = (i + j)/2$}
	\State{$mergeSort(A, i, mid)$} \Comment{first recursive call on left side}
	\State{$mergeSort(A, mid, j)$}  \Comment{second recursive call on right side}
	\State{$merge(A, i, mid, j)$}
	\EndFunction                           
	\end{algorithmic}
\end{algorithm}
